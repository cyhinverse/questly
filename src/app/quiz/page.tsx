...
export default function QuizList() {
  ...
  const filteredQuizzes = useMemo(() => {
    return quizzes.filter((quiz) => {
      ...
      const userMatch =
        filter === "my" && user ? quiz.created_by === user.id : true;
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 pt-24">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-black mb-4">Quiz Library</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
            {filteredQuizzes.map((quiz) => (
              <QuizCard
                ...
                onPlay={() => router.push(`/quiz/${quiz.id}`)}
                onPlaySolo={() => {
                  ...
                  const roomCode = Math.random()
                    ...
                    .substring(2, 8)
                    .toUpperCase();
                  router.push(`/room/lobby?create=${quiz.id}&code=${roomCode}`);
                }}
              />
            ))}
          </div>
