...
export default function CreateQuiz() {
    ...
    try {
      ...
      if (!process.env.NEXT_PUBLIC_OPENROUTER_API_KEY) {
        toast({
          title: "Error",
          description:
            "API key không được tìm thấy. Vui lòng kiểm tra cấu hình.",
          variant: "destructive",
        });
        return;
      }
      ...
      const aiQuestions = await generateQuestionsWithMistral(
        aiTopic,
        aiQuestionCount,
        randomDifficulty
      );
      ...
      if (!currentDescription?.trim()) {
        setValue(
          "quiz.description",
          `A quiz about ${aiTopic} with ${aiQuestionCount} questions`
        );
      }
  ...
  const generateQuestionsWithMistral = async (
    topic: string,
    count: number,
    difficulty: string
  ) => {
    ...
    for (let i = 0; i < batches; i++) {
      ...
      // Cập nhật progress
      setAiProgress(
        `Đang tạo batch ${batchNumber}/${batches} (${allQuestions.length}/${count} câu hỏi)...`
      );
      ...
      while (retryCount < maxRetries && !batchQuestions) {
        try {
          ...
          if (i < batches - 1) {
            setAiProgress(
              `Hoàn thành batch ${batchNumber}, chờ ${
                delay / 1000
              }s trước batch tiếp theo...`
            );
          ...
          if (retryCount < maxRetries) {
            ...
            if (isRateLimit) {
              ...
              setAiProgress(
                `Rate limit! Chờ ${
                  backoffDelay / 1000
                }s trước khi thử lại... (${retryCount}/${maxRetries})`
              );
              ...
              setAiProgress(
                `Batch ${batchNumber} thất bại, thử lại sau ${
                  backoffDelay / 1000
                }s... (${retryCount}/${maxRetries})`
              );
            }
            ...
            if (currentBatchSize > 3) {
              setAiProgress(
                `Batch ${batchNumber} thất bại hoàn toàn, thử với batch nhỏ hơn...`
              );
              try {
                ...
                throw new Error(
                  `Không thể tạo batch ${batchNumber} ngay cả với batch size nhỏ: ${smallerError}`
                );
              }
            } else {
              throw new Error(
                `Không thể tạo batch ${batchNumber} sau ${maxRetries} lần thử`
              );
            }
          }
        }
...
Tạo ${batchSize} câu hỏi ${difficulty} về ${topic}`,
    ...
    const validatedQuestions = questions.map(
      (
        q: { content?: string; options?: string[]; correct_answer?: number },
        index: number
      ) => ({
  ...
  const onSubmit = async (data: CreateQuizFormData) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: `Lỗi khi thêm câu hỏi: ${
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="text-center mb-6">
              <h1 className="text-4xl font-sans font-bold text-black mb-3 tracking-tight">
                Create New Quiz
              </h1>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger
                        className={
                          errors.quiz?.difficulty ? "border-red-500" : ""
                        }
                      >
                    ...
                    {errors.quiz?.difficulty && (
                      <p className="text-red-500 text-xs">
                        {errors.quiz.difficulty.message}
                      </p>
                    )}
                  </div>
                </div>
                <div className="mt-3 space-y-2">
                  ...
                  {errors.quiz?.description && (
                    <p className="text-red-500 text-xs">
                      {errors.quiz.description.message}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                {aiGenerating && (
                  <div className="text-blue-600 text-sm">
                    {aiProgress ||
                      `AI đang tạo ${aiQuestionCount} câu hỏi về "${aiTopic}"...`}
                  </div>
                )}
              </div>
              {/* Questions */}
                ...
                {fields.map((field, qIndex) => (
                  ...
                  >
                    ...
                    <div className="mb-6 space-y-2">
                      ...
                      {errors.questions?.[qIndex]?.content && (
                        <p className="text-red-500 text-xs">
                          {errors.questions[qIndex]?.content?.message}
                        </p>
                      )}
                    </div>
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {[0, 1, 2, 3].map((oIndex) => (
                          <div
                            key={oIndex}
                            className="flex items-center gap-3 bg-white border border-gray-200 p-3 rounded-lg hover:border-purple-300 transition-colors"
                          >
                            <input
                              type="radio"
                              name={`correct-${qIndex}`}
                              checked={
                                watch(`questions.${qIndex}.correct_answer`) ===
                                oIndex
                              }
                              onChange={() =>
                                setValue(
